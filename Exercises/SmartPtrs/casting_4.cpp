...
bool f(std::shared_ptr<const Base> i_sp_base) {
	...
	// 7 - Dynamic downcast to a shared_ptr to Devide object type 
...
int main(){
	...
	// 1 - Static downcast to a shared_ptr to Devide object type
	std::shared_ptr<Derived> sp_cast_from_base = std::static_pointer_cast<Derived>(sp_base);

	// 2 - Which method are we calling?
    assert(sp_cast_from_base->f()==?);

	// 3 - Check smart pointer count
	assert(sp_base.use_count() == ?);
	assert(sp_cast_from_base.use_count()==?);

	// 4 - Dynamic downcast to a shared_ptr to Devide object type
	std::shared_ptr<Derived> sp_dyna_cast_from_base = ?		

	// 5 - Check smart pointer validity
	assert(sp_dyna_cast_from_base==?);
	...
	// 8 - Fix my code and call the non const method on const object
	std::shared_ptr<const Derived> sp_const_derived(new Derived);
	?(sp_const_derived)->setter();
